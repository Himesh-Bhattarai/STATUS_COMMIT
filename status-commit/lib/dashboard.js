...
export function generateDashboard({ repoPath, commits, commitFiles, graph, edges, driftEvents, open = true }) {
  const outDir = path.join(repoPath, '.status-intel', 'reports');
  fs.mkdirSync(outDir, { recursive: true });
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  const htmlPath = path.join(outDir, `dashboard-${stamp}.html`);

  const payload = { commits, commitFiles, graph, edges, driftEvents };
